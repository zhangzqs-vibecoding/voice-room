...
export const App = ({ mediaDevices = navigator.mediaDevices, members = [], sessionFactory = () => new LiveKitSessionAdapter() }: Props) => {
  ...
  const refreshDevices = async () => {
    ...
    try {
      ...
      const inputs = all.filter((device) => device.kind === 'audioinput').map((device, index) => ({ deviceId: device.deviceId, label: device.label || `麦克风 ${index + 1}` }));
      const videoInputs = all.filter((device) => device.kind === 'videoinput').map((device, index) => ({ deviceId: device.deviceId, label: device.label || `摄像头 ${index + 1}` }));
      setDevices(inputs); setCameras(videoInputs);
      if (!deviceId && inputs[0]) setDeviceId(inputs[0].deviceId);
      if (!cameraDeviceId && videoInputs[0]) setCameraDeviceId(videoInputs[0].deviceId);
...
export const Room = ({ roomId, nickname, avatarId, listenOnly, members, deviceId = '', devices = [], cameraDeviceId = '', cameras = [], mode = 'voice', credentials, sessionFactory = () => new LiveKitSessionAdapter(), onDeviceChange, onCameraDeviceChange, onRefreshDevices, onReconnect, onLeave }: { roomId: string; nickname: string; avatarId: AvatarId; listenOnly: boolean; members: Member[]; deviceId?: string; devices?: Device[]; cameraDeviceId?: string; cameras?: Device[]; mode?: AudioMode; credentials?: JoinResponse; sessionFactory?: () => SessionAdapter; onDeviceChange?: (deviceId: string) => void; onCameraDeviceChange?: (deviceId: string) => void; onRefreshDevices?: () => void; onReconnect?: () => void; onLeave: () => void }) => {
  ...
  useEffect(() => {
    ...
    void next.enter({ ...credentials, listenOnly, mode, deviceId }).then(() => listenOnly ? undefined : next.startCamera(cameraDeviceId)).catch((error) => setControlError(error instanceof Error && error.message === 'video_not_supported' ? '摄像头不可用，请检查浏览器权限。' : '会议连接失败，请重试。'));
  ...
  const changeCamera = async (nextDeviceId: string) => { try { await session.current?.startCamera(nextDeviceId); onCameraDeviceChange?.(nextDeviceId); setControlError(''); } catch { setControlError('摄像头切换失败，请检查设备后重试。'); } };
  const participants: VideoParticipant[] = [{ id: 'self', name: self.name, avatar: AVATARS.find((item) => item.id === avatarId)?.icon ?? '🦊', track: state.videoTracks.self, cameraEnabled: state.localCameraEnabled, speaking: !muted && state.localSpeaking, handRaised }, ...visibleMembers.map((member) => ({ id: member.id, name: member.name, avatar: AVATARS.find((item) => item.id === member.avatarId)?.icon ?? '🦊', track: state.videoTracks[member.id], trackSid: member.cameraTrackSid, microphoneTrackSid: member.microphoneTrackSid, cameraEnabled: member.cameraEnabled !== false && Boolean(state.videoTracks[member.id]), speaking: member.speaking, handRaised: (state.raisedHands ?? []).includes(member.id) }))];
