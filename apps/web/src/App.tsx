...
interface Props { mediaDevices?: MediaDevicesLike; members?: Member[] }
...
export const App = ({ mediaDevices = navigator.mediaDevices, members = [] }: Props) => {
  ...
  const enter = async (nextListenOnly: boolean) => {
    ...
    try { await joinRoom(roomId, { nickname: validName, avatarId }); setListenOnly(nextListenOnly); setScreen('room'); }
  ...
  const copyInvite = async () => { try { await navigator.clipboard?.writeText(inviteUrl); setMessage('邀请链接已复制。'); } catch { setMessage('请手动复制邀请链接。'); } };
  if (screen === 'room') return <Room roomId={roomId} nickname={validateNickname(nickname) ?? '我'} avatarId={avatarId} listenOnly={listenOnly} members={members} onLeave={() => { setMessage(''); setScreen('lobby'); }} />;
...
export const Room = ({ roomId, nickname, avatarId, listenOnly, members, onLeave }: { roomId: string; nickname: string; avatarId: AvatarId; listenOnly: boolean; members: Member[]; onLeave: () => void }) => {
  const [muted, setMuted] = useState(false);
  const self: Member = { id: 'self', name: nickname, avatarId, speaking: false };
  return <main className="console room"><header><p className="eyebrow">ROOM / {roomId}</p><h1>声场已接通</h1><p className="subtitle">{listenOnly ? '仅收听 · 等待音频连接' : '等待麦克风与 RTC 音频连接'}</p></header><section className="participants" aria-label="房间成员">{[self, ...members].map((member) => { const avatar = AVATARS.find((item) => item.id === member.avatarId)!; const isSelf = member.id === 'self'; return <article className="participant" key={member.id} data-speaking={member.speaking}><div className="portrait">{avatar.icon}</div><div><b>{member.name}{isSelf ? '（我）' : ''}</b><p>{isSelf ? (listenOnly ? '仅收听 · 等待音频连接' : muted ? '已静音' : '等待音轨连接') : member.speaking ? '正在发言' : '静默'}</p></div>{member.speaking && <span className="meter" aria-label={`${member.name} 正在发言`}>●</span>}</article>; })}</section><div className="actions">{!listenOnly && <button className="outline" onClick={() => setMuted(!muted)} aria-pressed={muted}>{muted ? '解除静音' : '静音'}</button>}<button className="danger" onClick={onLeave}>离开房间</button></div></main>;
};
