...
export const App = ({ mediaDevices = navigator.mediaDevices, members = [], sessionFactory = () => new LiveKitSessionAdapter() }: Props) => {
  ...
  if (screen === 'room' && credentials) return <Room roomId={roomId} nickname={validateNickname(nickname) ?? '我'} avatarId={avatarId} listenOnly={listenOnly} members={members} deviceId={deviceId} mode={mode} credentials={credentials} sessionFactory={sessionFactory} onReconnect={() => void enter(listenOnly)} onLeave={() => { setMessage(''); setScreen('lobby'); }} />;
...
export const Room = ({ roomId, nickname, avatarId, listenOnly, members, deviceId = '', mode = 'voice', credentials, sessionFactory = () => new LiveKitSessionAdapter(), onReconnect, onLeave }: { roomId: string; nickname: string; avatarId: AvatarId; listenOnly: boolean; members: Member[]; deviceId?: string; mode?: AudioMode; credentials?: JoinResponse; sessionFactory?: () => SessionAdapter; onReconnect?: () => void; onLeave: () => void }) => {
