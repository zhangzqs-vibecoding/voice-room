...
export const Room = ({ roomId, nickname, avatarId, listenOnly, members, deviceId = '', devices = [], mode = 'voice', credentials, sessionFactory = () => new LiveKitSessionAdapter(), onDeviceChange, onRefreshDevices, onReconnect, onLeave }: { roomId: string; nickname: string; avatarId: AvatarId; listenOnly: boolean; members: Member[]; deviceId?: string; devices?: Device[]; mode?: AudioMode; credentials?: JoinResponse; sessionFactory?: () => SessionAdapter; onDeviceChange?: (deviceId: string) => void; onRefreshDevices?: () => void; onReconnect?: () => void; onLeave: () => void }) => {
  ...
  return <main className="console room"><header><p className="eyebrow">ROOM / {roomId}</p><h1>声场已接通</h1><p className="subtitle">{state.connection === 'reconnecting' ? '正在恢复连接…' : state.connection === 'disconnected' ? '连接已断开' : listenOnly ? '仅收听 · 已连接 RTC 音频' : 'RTC 音频已连接'}</p></header>{!listenOnly && <div className="input-grid room-device"><label>麦克风设备<select aria-label="房内麦克风设备" value={deviceId} onChange={(event) => void changeDevice(event.target.value)}><option value="">自动选择</option>{devices.map((device) => <option value={device.deviceId} key={device.deviceId}>{device.label}</option>)}</select></label><button className="refresh" onClick={onRefreshDevices}>↻ 刷新设备</button></div>}{controlError && <p role="alert" className="notice">{controlError}</p>}<section className="participants" aria-label="房间成员">{[self, ...visibleMembers].map((member) => { const avatar = AVATARS.find((item) => item.id === member.avatarId)!; const isSelf = member.id === 'self'; return <article className="participant" key={member.id} data-speaking={member.speaking}><div className="portrait">{avatar.icon}</div><div><b>{member.name}{isSelf ? '（我）' : ''}</b><p>{isSelf ? (listenOnly ? '仅收听' : muted ? '已静音' : member.speaking ? '正在发言' : '静默') : member.speaking ? '正在发言' : '静默'}</p></div>{member.speaking && <span className="meter" aria-label={`${member.name} 正在发言`}>●</span>}</article>; })}</section><div className="actions">{state.connection === 'disconnected' && onReconnect && <button className="primary" onClick={onReconnect}>重新入场</button>}{!listenOnly && <button className="outline" onClick={() => void toggleMute()} aria-pressed={muted}>{muted ? '解除静音' : '静音'}</button>}<button className="danger" onClick={() => void leave()}>离开房间</button></div></main>;
};
